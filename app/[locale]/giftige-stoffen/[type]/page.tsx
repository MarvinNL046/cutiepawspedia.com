...
const validTypes = ['plant', 'voedsel', 'medicijn', 'huishouden', 'chemisch'] as const;
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const substances = getSubstancesByType(type as ToxicSubstance['type']);

  const title = `Giftige ${typeInfo.name} voor Huisdieren | ${substances.length} Stoffen | CutiePawsPedia`;
  const description = `Compleet overzicht van ${substances.length} giftige ${typeInfo.name.toLowerCase()} voor honden, katten en andere huisdieren. ${typeInfo.description}`;
  ...
  return {
    ...
    openGraph: {
      title: `Giftige ${typeInfo.name} voor Huisdieren`,
      description,
      type: 'website',
...
export default async function ToxicityCategoryPage({ params }: PageProps) {
  const { locale, type } = await params;
  ...
  // Sort by toxicity level (hoog first) then by name
      ...
      <script
        ...
        dangerouslySetInnerHTML={{
          __html: JSON.stringify({
            ...
            '@type': 'CollectionPage',
            name: `Giftige ${typeInfo.name} voor Huisdieren`,
            description: `Overzicht van ${substances.length} giftige ${typeInfo.name.toLowerCase()} voor huisdieren.`,
      ...
      <article className="max-w-7xl mx-auto px-4 py-12">
        ...
        <header className="mb-12 text-center">
          ...
          <h1 className="text-4xl md:text-5xl font-bold text-gray-900 mb-4">
            Giftige {typeInfo.name} voor Huisdieren
          </h1>
          <p className="text-xl text-gray-600 max-w-3xl mx-auto mb-6">
            {typeInfo.description}. Hier vind je {substances.length} {typeInfo.name.toLowerCase()} die gevaarlijk kunnen zijn voor je huisdier.
          </p>
          ...
          <div className="flex items-start gap-4">
            ...
            <div>
              ...
              <p className="text-gray-800 leading-relaxed">
                Bij vermoedelijke vergiftiging: <strong>neem direct contact op met je dierenarts of de dierennoodhulp</strong>.
                Wacht niet tot symptomen erger worden. Deze informatie is bedoeld voor preventie, niet als vervanging van veterinair advies.
              </p>
            </div>
          </div>
        </div>
        ...
        <section className="mb-12">
          <h2 className="text-2xl font-bold text-gray-900 mb-6">
            Alle Giftige {typeInfo.name} ({substances.length})
          </h2>

          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
            {sortedSubstances.map((substance) => {
              const levelInfo = getToxicityLevelInfo(substance.toxicityLevel);
              // Get first animal for default link
              const firstAnimal = substance.animals[0] as AnimalType;
              const animalInfo = getAnimalInfo(firstAnimal);

              return (
                <Link
                  key={substance.slug}
                  href={`/${locale}/giftig/${substance.slug}/${firstAnimal}`}
                  ...
                  <div className="flex items-start justify-between gap-3">
                    <div className="flex-1">
                      ...
                      <p className="text-sm text-gray-600 mb-2 line-clamp-1">
                        {substance.notes || 'Kan gevaarlijk zijn voor huisdieren'}
                      </p>
                      <div className="flex flex-wrap gap-1">
                        {substance.animals.slice(0, 3).map(animal => {
                          const info = getAnimalInfo(animal as AnimalType);
                          return (
                            <span key={animal} className="text-xs bg-white/50 rounded px-1.5 py-0.5">
                              {info.icon} {info.plural}
                            </span>
                          );
                        })}
                        {substance.animals.length > 3 && (
                          <span className="text-xs bg-white/50 rounded px-1.5 py-0.5">
                            +{substance.animals.length - 3}
                          </span>
                        )}
                      </div>
        ...
        <section className="mb-12">
          ...
          <div className="grid md:grid-cols-4 gap-4">
            {validTypes
              .filter(t => t !== type)
              .map(otherType => {
                const info = getTypeInfo(otherType);
                const count = getSubstancesByType(otherType).length;
                return (
                  <Link
                    key={otherType}
                    href={`/${locale}/giftige-stoffen/${otherType}`}
                    className="bg-white rounded-lg shadow-md p-4 hover:shadow-lg transition-shadow"
                  >
                    <div className="text-3xl mb-2">{info.icon}</div>
                    <h3 className="font-semibold text-gray-900">{info.name}</h3>
                    <p className="text-sm text-gray-600">{count} stoffen</p>
                  </Link>
                );
              })}
          </div>
