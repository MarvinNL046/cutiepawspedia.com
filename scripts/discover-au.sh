#!/bin/bash
# =============================================================================
# Australia Place Discovery Script - With Retry & Rate Limit Protection
# =============================================================================
#
...
#
# Usage:
#   ./scripts/discover-au.sh [--category=<slug>] [--limit=<n>]
#
# Examples:
#   ./scripts/discover-au.sh                          # All categories
#   ./scripts/discover-au.sh --category=veterinary    # Single category
#   ./scripts/discover-au.sh --limit=10               # Limit results per city
#
# =============================================================================
...
# Progress tracking
PROGRESS_FILE="/tmp/discover-au-progress.txt"
LOG_FILE="/tmp/discover-au-$(date +%Y%m%d-%H%M%S).log"
...
while [[ $# -gt 0 ]]; do
  case $1 in
    ...
    --resume)
      ...
      ;;
    --help|-h)
      echo "Australia Place Discovery Script"
      echo ""
...
log() {
  ...
  echo -e "${timestamp} [Australia] ${msg}" | tee -a "$LOG_FILE"
}
...
run_discovery() {
  ...
  while [[ $attempt -le $MAX_RETRIES ]]; do
    ...
    set +e
    output=$(npx tsx scripts/discover-places-au.ts \
      --category="$category" \
      --all-cities \
      --limit="$LIMIT" \
      2>&1)
...
echo ""
echo -e "${GREEN}╔═══════════════════════════════════════════════════════════════╗${NC}"
echo -e "${GREEN}║        🇦🇺 Australia Place Discovery - Enhanced Edition             ║${NC}"
echo -e "${GREEN}║                                                               ║${NC}"
...
echo ""
echo -e "${GREEN}╔═══════════════════════════════════════════════════════════════╗${NC}"
echo -e "${GREEN}║              🇦🇺 Australia DISCOVERY COMPLETE                       ║${NC}"
echo -e "${GREEN}╚═══════════════════════════════════════════════════════════════╝${NC}"
echo ""
