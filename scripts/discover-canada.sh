#!/bin/bash
# =============================================================================
# Canada Place Discovery Script - With Retry & Rate Limit Protection
# =============================================================================
#
# Discovers places in all Canadian cities across all categories.
...
#
# Usage:
#   ./scripts/discover-canada.sh [--category=<slug>] [--limit=<n>]
#
# Examples:
#   ./scripts/discover-canada.sh                          # All categories
#   ./scripts/discover-canada.sh --category=veterinary    # Single category
#   ./scripts/discover-canada.sh --limit=10               # Limit results per city
#
# =============================================================================
...
# Progress tracking
PROGRESS_FILE="/tmp/discover-canada-progress.txt"
LOG_FILE="/tmp/discover-canada-$(date +%Y%m%d-%H%M%S).log"
...
while [[ $# -gt 0 ]]; do
  case $1 in
    ...
    --resume)
      ...
      ;;
    --help|-h)
      echo "Canada Place Discovery Script"
      echo ""
...
log() {
  ...
  echo -e "${timestamp} [CANADA] ${msg}" | tee -a "$LOG_FILE"
}
...
run_discovery() {
  ...
  while [[ $attempt -le $MAX_RETRIES ]]; do
    log "${MAGENTA}🔍 Running discovery for: ${category} (attempt ${attempt}/${MAX_RETRIES})${NC}"
    ...
    set +e
    output=$(npx tsx scripts/discover-places-ca.ts \
      --category="$category" \
      --all-cities \
      --limit="$LIMIT" \
      2>&1)
...
echo ""
echo -e "${MAGENTA}╔═══════════════════════════════════════════════════════════════╗${NC}"
echo -e "${MAGENTA}║        🇨🇦 Canada Place Discovery - Enhanced Edition          ║${NC}"
echo -e "${MAGENTA}║                                                               ║${NC}"
echo -e "${MAGENTA}║  Features:                                                    ║${NC}"
echo -e "${MAGENTA}║  • Retry logic with exponential backoff                       ║${NC}"
echo -e "${MAGENTA}║  • Rate limit detection & adaptive delays                     ║${NC}"
echo -e "${MAGENTA}║  • CIDs and Place IDs automatically saved                     ║${NC}"
echo -e "${MAGENTA}║  • Progress tracking for resume capability                    ║${NC}"
echo -e "${MAGENTA}╚═══════════════════════════════════════════════════════════════╝${NC}"
echo ""
...
echo ""
echo -e "${MAGENTA}╔═══════════════════════════════════════════════════════════════╗${NC}"
echo -e "${MAGENTA}║              🇨🇦 CANADA DISCOVERY COMPLETE                    ║${NC}"
echo -e "${MAGENTA}╚═══════════════════════════════════════════════════════════════╝${NC}"
echo ""
