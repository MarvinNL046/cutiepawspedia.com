#!/bin/bash
# =============================================================================
# Spain Place Discovery Script - With Retry & Rate Limit Protection
# =============================================================================
#
...
#
# Usage:
#   ./scripts/discover-es.sh [--category=<slug>] [--limit=<n>]
#
# Examples:
#   ./scripts/discover-es.sh                          # All categories
#   ./scripts/discover-es.sh --category=veterinary    # Single category
#   ./scripts/discover-es.sh --limit=10               # Limit results per city
#
# =============================================================================
...
# Progress tracking
PROGRESS_FILE="/tmp/discover-es-progress.txt"
LOG_FILE="/tmp/discover-es-$(date +%Y%m%d-%H%M%S).log"
...
while [[ $# -gt 0 ]]; do
  case $1 in
    ...
    --resume)
      ...
      ;;
    --help|-h)
      echo "Spain Place Discovery Script"
      echo ""
...
log() {
  ...
  echo -e "${timestamp} [Spain] ${msg}" | tee -a "$LOG_FILE"
}
...
run_discovery() {
  ...
  while [[ $attempt -le $MAX_RETRIES ]]; do
    ...
    set +e
    output=$(npx tsx scripts/discover-places-es.ts \
      --category="$category" \
      --all-cities \
      --limit="$LIMIT" \
      2>&1)
...
echo ""
echo -e "${GREEN}╔═══════════════════════════════════════════════════════════════╗${NC}"
echo -e "${GREEN}║        🇪🇸 Spain Place Discovery - Enhanced Edition             ║${NC}"
echo -e "${GREEN}║                                                               ║${NC}"
...
echo ""
echo -e "${GREEN}╔═══════════════════════════════════════════════════════════════╗${NC}"
echo -e "${GREEN}║              🇪🇸 Spain DISCOVERY COMPLETE                       ║${NC}"
echo -e "${GREEN}╚═══════════════════════════════════════════════════════════════╝${NC}"
echo ""
