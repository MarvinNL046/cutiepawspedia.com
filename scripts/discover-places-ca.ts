#!/usr/bin/env npx tsx
/**
 * Discover Places in Canada via BrightData SERP API
 *
 * Usage:
 *   npx tsx scripts/discover-places-ca.ts --category=veterinary --city=toronto
 *   npx tsx scripts/discover-places-ca.ts --category=dog-training --all-cities
 *   npx tsx scripts/discover-places-ca.ts --category=pet-hotel --city=vancouver --dry-run
 */
...
Usage:
  npx tsx scripts/discover-places-ca.ts --category=<slug> --city=<name>
  npx tsx scripts/discover-places-ca.ts --category=<slug> --all-cities
  npx tsx scripts/discover-places-ca.ts --help

Options:
  -c, --category <slug>   Category to search (required)
  -t, --city <name>       City slug (e.g., toronto, vancouver, montreal)
  -a, --all-cities        Search in all Canadian cities
...
Examples:
  npx tsx scripts/discover-places-ca.ts --category=veterinary --city=toronto
  npx tsx scripts/discover-places-ca.ts --category=grooming --all-cities --limit=10
`);
  process.exit(0);
}
...
async function getCanadianCities(citySlug?: string): Promise<CityInfo[]> {
  const canadaCountry = await db
    .select()
    .from(schema.countries)
    .where(eq(schema.countries.code, "CA"))
    .limit(1);

  if (canadaCountry.length === 0) {
    throw new Error("Canada not found in database. Run add-usa-canada-cities.ts first.");
  }

  const countryId = canadaCountry[0].id;
...
async function searchGoogleMaps(
  ...
  const searchUrl = `https://www.google.ca/search?q=${encodeURIComponent(query + " " + city)}&tbm=lcl&hl=en&gl=ca&num=${limit}`;
...
function parseSerpResults(data: object): LocalResult[] {
  ...
  for (const r of localResults) {
    ...
    results.push({
      ...
      cid: String(r.cid || r.data_id || ""),           // Google CID (numeric)
      placeId: String(r.place_id || ""),               // Google Place ID (ChIJ...)
...
async function createPlace(
  ...
  try {
    const scrapedContent: Record<string, unknown> = {
      googleCid: result.cid || null,           // Store CID in scraped content for reference
      googlePlaceId: result.placeId || null,   // Store Place ID in scraped content for reference
      googleRating: result.rating,
      googleReviewCount: result.reviews,
      category: result.category,
      discoveredAt: new Date().toISOString(),
      discoverySource: "brightdata_serp_api_ca",
    };
...
async function main() {
  ...
  const cities = await getCanadianCities(CITY);
  if (cities.length === 0) {
  ...
  // Summary
  console.log("\n" + "=".repeat(60));
  console.log("🇨🇦 CANADA DISCOVERY SUMMARY");
