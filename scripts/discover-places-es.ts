#!/usr/bin/env npx tsx
/**
 * Discover Places in Spain via BrightData SERP API
 *
 * Usage:
...
Options:
  ...
  -a, --all-cities        Search in all ES cities
...
async function getESCities(citySlug?: string): Promise<CityInfo[]> {
  const usaCountry = await db
    .select()
    .from(schema.countries)
    .where(eq(schema.countries.code, "ES"))
    .limit(1);

  if (usaCountry.length === 0) {
    throw new Error("Spain not found in database. Run add-usa-canada-cities.ts first.");
  }
...
async function main() {
  ...
  const cities = await getESCities(CITY);
  if (cities.length === 0) {
  ...
  // Summary
  console.log("\n" + "=".repeat(60));
  console.log("🇺🇸 Spain DISCOVERY SUMMARY");
