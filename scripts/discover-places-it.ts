#!/usr/bin/env npx tsx
/**
 * Discover Places in Italy via BrightData SERP API
 *
 * Usage:
...
Options:
  ...
  -a, --all-cities        Search in all IT cities
...
async function getITCities(citySlug?: string): Promise<CityInfo[]> {
  const usaCountry = await db
    .select()
    .from(schema.countries)
    .where(eq(schema.countries.code, "IT"))
    .limit(1);

  if (usaCountry.length === 0) {
    throw new Error("Italy not found in database. Run add-usa-canada-cities.ts first.");
  }
...
async function createPlace(
  ...
  try {
    const scrapedContent: Record<string, unknown> = {
      ...
      discoverySource: "brightdata_serp_api_it",
    };
...
async function main() {
  ...
  const cities = await getITCities(CITY);
  if (cities.length === 0) {
  ...
  // Summary
  console.log("\n" + "=".repeat(60));
  console.log("🇺🇸 Italy DISCOVERY SUMMARY");
