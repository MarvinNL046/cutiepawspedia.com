#!/usr/bin/env npx tsx
/**
 * Discover Places in UK via BrightData SERP API
 *
 * Usage:
...
Options:
  ...
  -a, --all-cities        Search in all UK cities
...
async function getUKCities(citySlug?: string): Promise<CityInfo[]> {
  const usaCountry = await db
    .select()
    .from(schema.countries)
    .where(eq(schema.countries.code, "GB"))
    .limit(1);

  if (usaCountry.length === 0) {
    throw new Error("UK not found in database. Run add-usa-canada-cities.ts first.");
  }
...
async function createPlace(
  ...
  try {
    const scrapedContent: Record<string, unknown> = {
      ...
      discoverySource: "brightdata_serp_api_uk",
    };
...
async function main() {
  ...
  const cities = await getUKCities(CITY);
  if (cities.length === 0) {
  ...
  // Summary
  console.log("\n" + "=".repeat(60));
  console.log("🇺🇸 UK DISCOVERY SUMMARY");
