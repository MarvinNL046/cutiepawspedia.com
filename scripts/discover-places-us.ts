#!/usr/bin/env npx tsx
/**
 * Discover Places in USA via BrightData SERP API
 *
 * Usage:
...
Options:
  ...
  -a, --all-cities        Search in all US cities
...
async function getUSCities(citySlug?: string): Promise<CityInfo[]> {
  const usaCountry = await db
    .select()
    .from(schema.countries)
    .where(eq(schema.countries.code, "US"))
    .limit(1);

  if (usaCountry.length === 0) {
    throw new Error("USA not found in database. Run add-usa-canada-cities.ts first.");
  }
...
async function main() {
  ...
  const cities = await getUSCities(CITY);
  if (cities.length === 0) {
  ...
  // Summary
  console.log("\n" + "=".repeat(60));
  console.log("🇺🇸 USA DISCOVERY SUMMARY");
