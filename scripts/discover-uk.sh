#!/bin/bash
# =============================================================================
# United Kingdom Place Discovery Script - With Retry & Rate Limit Protection
# =============================================================================
#
...
#
# Usage:
#   ./scripts/discover-uk.sh [--category=<slug>] [--limit=<n>]
#
# Examples:
#   ./scripts/discover-uk.sh                          # All categories
#   ./scripts/discover-uk.sh --category=veterinary    # Single category
#   ./scripts/discover-uk.sh --limit=10               # Limit results per city
#
# =============================================================================
...
# Progress tracking
PROGRESS_FILE="/tmp/discover-uk-progress.txt"
LOG_FILE="/tmp/discover-uk-$(date +%Y%m%d-%H%M%S).log"
...
while [[ $# -gt 0 ]]; do
  case $1 in
    ...
    --resume)
      ...
      ;;
    --help|-h)
      echo "United Kingdom Place Discovery Script"
      echo ""
...
log() {
  ...
  echo -e "${timestamp} [United Kingdom] ${msg}" | tee -a "$LOG_FILE"
}
...
run_discovery() {
  ...
  while [[ $attempt -le $MAX_RETRIES ]]; do
    ...
    set +e
    output=$(npx tsx scripts/discover-places-uk.ts \
      --category="$category" \
      --all-cities \
      --limit="$LIMIT" \
      2>&1)
...
echo ""
echo -e "${GREEN}╔═══════════════════════════════════════════════════════════════╗${NC}"
echo -e "${GREEN}║        🇬🇧 United Kingdom Place Discovery - Enhanced Edition             ║${NC}"
echo -e "${GREEN}║                                                               ║${NC}"
...
echo ""
echo -e "${GREEN}╔═══════════════════════════════════════════════════════════════╗${NC}"
echo -e "${GREEN}║              🇬🇧 United Kingdom DISCOVERY COMPLETE                       ║${NC}"
echo -e "${GREEN}╚═══════════════════════════════════════════════════════════════╝${NC}"
echo ""
