#!/bin/bash
# =============================================================================
# USA Place Discovery Script - With Retry & Rate Limit Protection
# =============================================================================
#
...
#
# Usage:
#   ./scripts/discover-usa.sh [--category=<slug>] [--limit=<n>]
#
# Examples:
#   ./scripts/discover-usa.sh                          # All categories
#   ./scripts/discover-usa.sh --category=veterinary    # Single category
#   ./scripts/discover-usa.sh --limit=10               # Limit results per city
#
# =============================================================================
...
# Progress tracking
PROGRESS_FILE="/tmp/discover-usa-progress.txt"
LOG_FILE="/tmp/discover-usa-$(date +%Y%m%d-%H%M%S).log"
...
while [[ $# -gt 0 ]]; do
  case $1 in
    ...
    --resume)
      ...
      ;;
    --help|-h)
      echo "USA Place Discovery Script"
      echo ""
...
log() {
  ...
  echo -e "${timestamp} [USA] ${msg}" | tee -a "$LOG_FILE"
}
...
run_discovery() {
  ...
  while [[ $attempt -le $MAX_RETRIES ]]; do
    ...
    set +e
    output=$(npx tsx scripts/discover-places-us.ts \
      --category="$category" \
      --all-cities \
      --limit="$LIMIT" \
      2>&1)
...
echo ""
echo -e "${GREEN}╔═══════════════════════════════════════════════════════════════╗${NC}"
echo -e "${GREEN}║        🇺🇸 USA Place Discovery - Enhanced Edition             ║${NC}"
echo -e "${GREEN}║                                                               ║${NC}"
...
echo ""
echo -e "${GREEN}╔═══════════════════════════════════════════════════════════════╗${NC}"
echo -e "${GREEN}║              🇺🇸 USA DISCOVERY COMPLETE                       ║${NC}"
echo -e "${GREEN}╚═══════════════════════════════════════════════════════════════╝${NC}"
echo ""
