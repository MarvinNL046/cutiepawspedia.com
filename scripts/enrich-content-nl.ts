/**
 * AI Content Enrichment Script - NETHERLANDS ONLY
 *
 * Uses OpenAI to generate high-quality aboutUs content and descriptions
 * for Dutch places that have poor or missing content.
 *
 * Processes in batches to avoid memory issues.
 *
 * Gebruik: npx tsx scripts/enrich-content-nl.ts [--batch-size=50] [--offset=0]
 */
 ...
 */
async function main() {
  console.log("🇳🇱 AI Content Enrichment - NETHERLANDS ONLY\n");
  ...
  // Count total Netherlands places needing content
  ...
  console.log(`📊 Total Netherlands places needing content: ${totalNeeding}`);
  console.log(`📦 Processing batch: offset=${offset}, size=${batchSize}\n`);

  // Get batch of Netherlands places needing content
  ...
  if (remaining > 0) {
    console.log(`\n📋 Next batch command:`);
    console.log(`   npx tsx scripts/enrich-content-nl.ts --offset=${offset + batchSize} --batch-size=${batchSize}`);
  } else {
    console.log(`\n🎉 All Netherlands places have been enriched!`);
  }
}
