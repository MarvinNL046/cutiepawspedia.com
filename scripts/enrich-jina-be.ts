...
function extractDataFromContent(content: string, title: string): JinaExtracted {
  ...
  // Extract About Us - look for common patterns
  const aboutPatterns = [
    /(?:over ons|about us|wie zijn wij|onze missie|welkom bij)[:\s]*([^.!?]*(?:[.!?][^.!?]*){2,5})/i,
    /(?:wij zijn|we zijn|ons bedrijf)[:\s]*([^.!?]*(?:[.!?][^.!?]*){2,5})/i,
  ];
  ...
  const servicePatterns = [
    // Dutch
    /(?:trimmen|grooming|knippen|wassen|nagels)/gi,
    /(?:vaccinatie|inenting|sterilisatie|castratie)/gi,
    /(?:training|cursus|puppyschool|gehoorzaamheid)/gi,
    /(?:pension|opvang|dagopvang|logeren)/gi,
    /(?:consult|behandeling|operatie|spoed)/gi,
    /(?:voeding|dieet|supplementen)/gi,
    // French
    /(?:toilettage|bain|coupe)/gi,
    /(?:vaccination|stérilisation|castration)/gi,
    /(?:dressage|éducation|cours)/gi,
    /(?:pension|garde|garderie)/gi,
    /(?:consultation|traitement|chirurgie)/gi,
  ];
  ...
  const teamMatch = cleanContent.match(
    /(\d+)\s*(?:medewerkers|werknemers|mensen|personen|team|employees)/i
  );
  if (teamMatch) {
    facts.teamSize = `${teamMatch[1]} medewerkers`;
  }

  // Founded year
  const yearMatch = cleanContent.match(
    /(?:opgericht|sinds|gestart|founded|established|since)\s*(?:in\s*)?(\d{4})/i
  );
  if (yearMatch) {
  ...
  const hoursPattern =
    /(?:ma|di|wo|do|vr|za|zo|maandag|dinsdag|woensdag|donderdag|vrijdag|zaterdag|zondag|monday|tuesday|wednesday|thursday|friday|saturday|sunday)[:\s]*(\d{1,2}[:.]\d{2})\s*[-–]\s*(\d{1,2}[:.]\d{2})/gi;
 ...
 */
async function main() {
  console.log("🇧🇪 Jina Website Scraping - BELGIUM ONLY\n");
  ...
  // Count total Belgium places needing Jina scraping
  ...
  console.log(`\n📊 Belgium places with website needing Jina: ${totalNeeding}`);
  console.log(`📦 Processing batch: offset=${offset}, size=${batchSize}\n`);

  // Get batch of Belgium places
  ...
  if (remaining > 0) {
    console.log(`\n📋 Next batch command:`);
    console.log(
      `   npx tsx scripts/enrich-jina-be.ts --offset=${offset + batchSize} --batch-size=${batchSize}`
    );
  } else {
    console.log(`\n🎉 All Belgium places with websites have been scraped!`);
  }
}
