#!/usr/bin/env npx tsx
/**
 * Jina Website Scraping - NETHERLANDS ONLY
 *
 ...
 *
 * Usage:
 *   npx tsx scripts/enrich-jina-nl.ts [--batch-size=50] [--offset=0]
 */
...
function extractDataFromContent(content: string, title: string): JinaExtracted {
  ...
  // Extract About Us - look for common patterns (Dutch focused)
  const aboutPatterns = [
  ...
  // Extract services - look for service-related keywords (Dutch)
 ...
 */
async function main() {
  console.log("🇳🇱 Jina Website Scraping - NETHERLANDS ONLY\n");
  ...
  // Count total Netherlands places needing Jina scraping
  ...
  console.log(`\n📊 Netherlands places with website needing Jina: ${totalNeeding}`);
  console.log(`📦 Processing batch: offset=${offset}, size=${batchSize}\n`);

  // Get batch of Netherlands places
  ...
  if (remaining > 0) {
    console.log(`\n📋 Next batch command:`);
    console.log(
      `   npx tsx scripts/enrich-jina-nl.ts --offset=${offset + batchSize} --batch-size=${batchSize}`
    );
  } else {
    console.log(`\n🎉 All Netherlands places with websites have been scraped!`);
  }
}
