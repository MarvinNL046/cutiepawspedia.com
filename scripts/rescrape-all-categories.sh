#!/bin/bash
#
# Rescrape Google CIDs for all categories
# With progress bar, rate limiting, and retry logic
#
# Usage:
#   ./scripts/rescrape-all-categories.sh
#   ./scripts/rescrape-all-categories.sh --limit 100
#   ./scripts/rescrape-all-categories.sh --delay 2000
#
...
echo ""
echo -e "${BOLD}${BLUE}╔══════════════════════════════════════════════════════════════╗${NC}"
echo -e "${BOLD}${BLUE}║     🐾 CutiePawsPedia - Rescrape All Categories 🐾           ║${NC}"
echo -e "${BOLD}${BLUE}╚══════════════════════════════════════════════════════════════╝${NC}"
echo ""
...
for i in "${!CATEGORIES[@]}"; do
  ...
  echo ""
  echo -e "${YELLOW}📦 Processing: ${BOLD}${CATEGORY}${NC}"
  echo -e "${BOLD}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
  ...
  while [[ $RETRY_COUNT -lt $MAX_RETRIES ]] && [[ "$SUCCESS" == "false" ]]; do
    ...
    # Run the rescrape script
    if npx tsx scripts/rescrape-place-ids.ts --category "$CATEGORY" --limit "$LIMIT" --delay "$DELAY" 2>&1; then
      SUCCESS=true
  ...
  if [[ "$SUCCESS" == "false" ]]; then
    FAILED=$((FAILED + 1))
    echo -e "${RED}❌ ${CATEGORY} failed after ${MAX_RETRIES} retries${NC}"
  fi
...
# Check final stats
echo -e "${CYAN}📊 Final database stats:${NC}"
npx tsx scripts/check-place-ids.ts
