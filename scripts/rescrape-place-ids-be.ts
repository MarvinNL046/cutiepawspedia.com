/**
 * Re-scrape Google Place IDs and CIDs for existing Belgian places
 *
 ...
 *
 * Usage:
 *   npx tsx scripts/rescrape-place-ids-be.ts --category veterinary --limit 100
 *   npx tsx scripts/rescrape-place-ids-be.ts --category pet-store --delay 2000
 *   npx tsx scripts/rescrape-place-ids-be.ts --all --limit 50
 */
...
// Using google.be with gl=be for Belgium
async function searchPlace(
  ...
  // Using google.be with hl=nl (Dutch) and gl=be (Belgium)
  const searchUrl = `https://www.google.be/search?q=${encodeURIComponent(searchQuery)}&tbm=lcl&hl=nl&gl=be&num=5`;
...
async function main() {
  ...
  console.log("\n🔍 Re-scrape Place IDs Script (BELGIUM 🇧🇪)\n");
  ...
  if (category) {
    console.log(`📂 Fetching Belgian places in category: ${category}`);
    places = await sql`
      SELECT DISTINCT
        p.id,
        p.name,
    ...
    console.log("📂 Fetching all Belgian places without IDs");
    places = await sql`
      SELECT
        p.id,
        p.name,
  ...
  console.log(`\n📊 Found ${places.length} Belgian places to process\n`);

  if (places.length === 0) {
    console.log("✅ No Belgian places need updating!");
    process.exit(0);
  }

  if (dryRun) {
    console.log("🔍 Dry run - showing first 10 Belgian places that would be processed:");
