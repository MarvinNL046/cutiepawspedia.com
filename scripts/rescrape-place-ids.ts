/**
 * Re-scrape Google Place IDs and CIDs for existing places
 *
 ...
 *
 * Usage:
 *   npx tsx scripts/rescrape-place-ids.ts --category veterinary --limit 100
 *   npx tsx scripts/rescrape-place-ids.ts --category pet-store --delay 2000
 *   npx tsx scripts/rescrape-place-ids.ts --all --limit 50
 */
...
async function searchPlace(
  ...
  const searchUrl = `https://www.google.nl/search?q=${encodeURIComponent(searchQuery)}&tbm=lcl&hl=nl&gl=nl&num=5`;
...
async function main() {
  ...
  if (category) {
    console.log(`📂 Fetching places in category: ${category}`);
    places = await sql`
      SELECT DISTINCT
        p.id,
        p.name,
    ...
    console.log("📂 Fetching all places without IDs");
    places = await sql`
      SELECT
        p.id,
        p.name,
  ...
  if (places.length === 0) {
    console.log("✅ No places need updating!");
    process.exit(0);
  }

  if (dryRun) {
    console.log("🔍 Dry run - showing first 10 places that would be processed:");
